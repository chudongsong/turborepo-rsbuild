			...
			<el-popover placement="bottom-start" width="280" trigger="click" popper-class="el-tooltip-white" @hide="popoverHideEvent">
				<div class="flex flex-col text-secondary p-12px">
					<div class="flex items-center">
						...
						<el-switch v-model="refreshData.status" @change.prev="handleChangeTimeRefresh" />
					</div>
...
const saveRefresh = async () => {
	if (!/^\d+$/.test(String(refreshData.time)) || refreshData.time < 1) {
		Message.error('时间间隔不能小于1秒,且只能为正整数')
		return
	}
...
const clear = () => {
	...
	refreshData.time = props.sessionConfig?.time || 5
	isInit.value = true
	percentage.value = 100
	realRefreshTime.value = props.sessionConfig?.time || 5
}
