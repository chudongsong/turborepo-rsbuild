<template>
	<el-form ref="bindUserInfo" size="large" :rules="rules" :model="userInfo" autocomplete="off" class="bind-user-info" :class="{ 'bind-form': compData?.title ? true : false }">
		<div v-show="bindType === 1">
			<el-form-item class="text-center !mb-5 flex justify-center">
				<div class="flex !justify-center !w-full items-center">
					<h3 class="text-default inline-block mr-2 font-bold mt-12x">
			...
			<el-form-item v-show="isCheck" prop="code">
				<el-input v-model="userInfo.code" type="text" clearable autocomplete="off" placeholder="验证码" @keyup.enter.native="bindBtUser()">
					<template #append>
						<el-button class="relative top-0 rounded-r-base" type="primary" size="default" :disabled="formDisabled.code" plain @click="getVerifyCode">{{ tipsText }}</el-button>
					</template>
				</el-input>
			</el-form-item>
			<el-button type="primary" class="w-[100%] mt-[1.2rem]" size="default" @click="bindBtUser()">{{ compData?.btn || (compData?.title ? '绑定账号' : '切换账号') }}</el-button>
		...
		<div v-show="bindType === 2">
			<div v-if="!compData?.title" :class="{ 'view-con': true, 'bg-white': !compData?.title }">
				<div :class="{ 'app-box': true, 'bg-white': !compData?.title }">
			...
			<div
				...
				<div v-if="compData?.title" class="view-con bg-white">
					<div class="app-box bg-white">
						<img alt="" src="/static/images/other/bt_logo_new.png" />
					</div>
					<div class="change-icon">
						<svg t="1729493717851" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="19919" width="200" height="200">
				...
				<div v-if="!aliyunEcsLtd" class="w-[100%] mt-[1.2rem] flex items-center justify-center text-primary" size="default">
					<template v-if="!payment.bindUser">
						<bt-link @click="skipBinding">暂不绑定</bt-link>
						<bt-divider />
					</template>
		...
		<template v-if="bindType === 1">
			<div>
				...
				<div class="more-bind">
					...
					<div v-show="bindType === 1 && loginAuth" class="wechat-login-container" @click="changeBindType(2)">
						<div class="wechat-icon-circle">
							<i class="wechat-icon tencent_ico">
							</i>
						</div>
						<div class="wechat-text">阿里云</div>
					</div>
					<div v-show="bindType === 1" class="wechat-login-container" :class="{'ml-[1.4rem]': loginAuth}" @click="openWechatLogin">
						<div class="wechat-icon-circle">
							<i class="wechat-icon">
...
import { getBindCode, tencentLogin, checkLoginAuth as checkAliyunLoginAuth, getWechatAuthUrl, setWechatBindUser } from '@api/global'
...
const { aliyunEcsLtd, payment } = useGlobalStore()
...
const bindBtUser = async () => {
	...
	await bindUserRequest({ isCheck, userInfo, userToken })
}
...
const checkLoginAuth = async () => {
	try {
		...
		// if (res.status) {
		loginAuth.value = authData.value.client === 2 && authData.value.login === 1
		if (authData.value.client === 2 && authData.value.login === 1) {
			changeBindType(2)
		} else {
			changeBindType(1)
		}
