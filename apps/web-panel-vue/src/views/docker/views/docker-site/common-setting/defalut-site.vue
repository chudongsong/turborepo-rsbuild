<template>
	<div class="p-[20px]" v-bt-loading="loading">
		...
		<el-button type="primary" @click="handleSaveEvent" class="!mt-[20px] !block">保存</el-button>
		<bt-help
...
import { getDefaultSite, setDefaultSite } from '@api/docker'
...
const getDefaultData = async () => {
	...
	try {
		...
		defaultValue.value = data.data.defaultSite
		if (!data.data.defaultSite) defaultValue.value = '未设置默认站点'
		SiteOptions.value = data.data.sites
...
const handleSaveEvent = async () => {
	const params = {
		site_name: defaultValue.value === '未设置默认站点' ? '0' : defaultValue.value,
	}
	loading.value = true
	await useDataHandle({
