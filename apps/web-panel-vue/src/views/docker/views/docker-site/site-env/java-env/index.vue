...
import { openJavaForm } from './useController'
...
const { refreshJavaEnvTable } = storeToRefs(store)
...
const { BtTable, BtPage, BtBatch, refresh } = useAllTable({
	...
	columns: [
						...
						{{
							default: () => (
								<div class="flex items-center">
									<span class="bt-link cursor-pointer" onClick={() => setStatus(row.status === 'running' ? 'stop' : 'start', row, 'java')}>
										{row.status === 'running' ? '停止' : '启动'}
									</span>
									<BtDivider></BtDivider>
									<span class="bt-link cursor-pointer" onClick={() => setStatus('restart', row, 'java')}>
										重启
									</span>
								</div>
							),
							reference: () =>
								h(
									'span',
									{
										...
										onClick: () => {
											setStatus(row.status === 'running' ? 'stop' : 'start', row, 'java')
										},
									},
		...
		},
		{
			...
			render: (row: any) => {
				return (
					<span class="btlink truncate inline-block max-w-[20rem] cursor-pointer leading-[3.7rem]" title={row.path} onClick={() => openPathEvent(row, 'path')}>
						{row.path}
					</span>
				)
			},
		...
		useOperate([
			{
				...
				onClick: (row: any) => {
					createSite({ type: 'runEnv', formData: { type: 'java', env: `${row.name}:${row.version}` } })
				},
				title: '创建网站',
			},
			{
	...
	extension: [
		useTableBatch, // 批量操作
		useRefreshList(refreshJavaEnvTable), // 刷新
	],
})
