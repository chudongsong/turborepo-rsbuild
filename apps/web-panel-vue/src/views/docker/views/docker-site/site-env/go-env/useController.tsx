 ...
 * @returns {App}
 */
export const openGoForm = (row?: any): Promise<boolean> => {
	...
	return new Promise((resolve, error) => {
		useDialog({
			...
			area: 80,
			btn: ['确认', '取消'],
			component: () => import('@docker/views/docker-site/site-env/go-env/go-form.vue'),
			onCancel: () => {
				resetData()
				resolve(false)
			},
...
export const getFormData = async (dataRef: Ref<{ version: string }>): Promise<any> => {
	try {
		const res: any = await getVersion('go')
		if (res.status) {
			versionOptions.value = res.data['go'].map((item: any) => ({ label: item, value: item }))
 ...
 * @description 创建环境
 */
export const createGoEnv = async (formData: any, close: AnyFunction): Promise<any> => {
	try {
		...
		await useDataHandle({
			...
			success: (res: any) => {
				if (res.status) {
					close()
					openBuildLog(`tail -f /www/dk_project/runtime/run/go/${formData.name}/create.log`, () => {
						setTypeRefreshTable('go')
					})
				} else {
