		...
		<el-form ref="logFormRef" class="h-[48rem]" :model="logData" label-width="60px" v-bt-loading="formLoading" :rules="logRules" :disabled="formDisabled">
			...
			<div v-show="logData.type === 'file'" class="mt-[1.6rem]">
				<el-form-item label="访问日志" prop="accessLog">
					<bt-input v-model="logData.accessLog" placeholder="请选择或输入目录，例如：/www/wwwlogs" width="35rem">
						<template #append>
							<el-button @click="openPathDialog('accessLog')">
								<BtIcon icon="el-folder-opened" class="cursor-pointer" />
							</el-button>
						</template>
					</bt-input>
				</el-form-item>
...
import { globalConfig, formLoading, formDisabled, logRules, logFormRef, logData, typeOptions, logHelpList, openPathDialog, initLogData, onConfirmLog as onConfirm } from '@docker/views/docker-site/setting/global-config/useController'
