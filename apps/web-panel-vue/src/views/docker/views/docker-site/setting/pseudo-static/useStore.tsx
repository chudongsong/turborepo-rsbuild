import { delRewriteTel, getRewrite,getRewriteLists, setRewriteTel } from '@/api/docker';
...
const SITE_PSEUDO_STORE = defineStore('SITE-PSEUDO-STORE', () => {
  ...
  const getReWriteListEvent = async (params: any) => {
    ...
    try {
      ...
      const { data } = await useDataHandle({
        request: getRewriteLists(params),
      });
      return { data, status };
    } catch (error) {
      console.log(error);
   ...
   * @param path
   */
  const getReWriteEvent = async (params: any) => {
    // const loading = Message.load('正在获取伪静态列表，请稍候...');
    try {
      // 不验证 因为 sitePath 可能不会返回
      const { data } = await useDataHandle({
        request: getRewrite(params),
      });
      return { data, status };
    } catch (error) {
      console.log(error);
