import {
	...
	proxyGlobalAddBasic,
	proxyGlobalAddLimit,
	proxyGlobalBatchDelLimit,
	proxyGlobalCache,
	proxyGlobalCompress,
	proxyGlobalDelBasic,
	proxyGlobalDelLimit,
	proxyGlobalEditBasic,
	proxyGlobalSetLogs,
	proxyGlobalSetWebsocket,
	set404Status,
	get404Content,
	set404Content,
	seIpv6Status,
...
export const tabActive = ref('cache')
	...
	return [
		{
			...
			event: async (batchConfirm, nextAll, selectedList, options) => {
				deleteEvent(selectedList.value)
			},
		},
	]
}
...
export const addIpView = () => {
	...
	useDialog({
		...
		area: 40,
		btn: '确认',
		component: () => import('@docker/views/docker-site/setting/global-config/add-ip.vue'),
		compData: {
...
export const editHttpAuthEvent = (row?: any) => {
	...
	useDialog({
		...
		area: 48,
		btn: '确认',
		component: () => import('@docker/views/docker-site/setting/global-config/http-auth-add.vue'),
		compData: {
 ...
 * @description: 提交表单
 */
export const onConfirmAuth = async (close?: Function) => {
 ...
 */
export const setErrorEvent = (status: boolean) => {
	useDataHandle({
		loading: '正在设置，请稍后...',
		request: set404Status({ site_name: siteInfo.value.name, status: status ? '1' : '0' }),
		message: true,
	})
}
 ...
 * @description 获取编辑器内容
 */
export const getEditorContent = async (loading: Ref<boolean>, editorContent: Ref<string>, codeType: Ref<string>, isError: Ref<boolean>) => {
	try {
		const res: any = await useDataHandle({
			loading,
			request: get404Content({ site_name: siteInfo.value.name }),
			// message: true,
		})
		editorContent.value = res.data.data
		codeType.value = res.data.encoding || 'utf-8'
		isError.value = res.data.status
	} catch (error) {
		console.log(error)
	}
}
/**
 * @description 保存404编辑器内容
 */
export const save404File = async (content: string, code: string) => {
	useDataHandle({
		request: set404Content({ site_name: siteInfo.value.name, file_body: content, encoding: code }),
		message: true,
	})
}
/**************** 404 end *********************/
 ...
 * @param close
 */
export const onConfirmIpv6 = async () => {
	let params = {
		site_name: siteInfo.value.name,
		ipv6_status: globalConfig.value.ipv6_status ? '1' : '0',
	}
	await cacheFormRef.value.validate()
	await useDataHandle({
		loading: formDisabled,
		request: seIpv6Status(params),
		message: true,
	})
}
/**************** ipv6 end *********************/
