			...
			<el-table height="400" :data="uploadFilesList">
				<el-table-column prop="name" :show-overflow-tooltip="true" label="名称">
					<template #default="scope">
						<i class="text-supplement svgtofont-el-document" />
				...
				<template #empty>
					<el-empty :description="`只支持'${accept}'文件格式导入`" />
				</template>
			</el-table>
		</div>
...
import { getByteUnit, getToken } from '@/utils'
...
interface Props {
	compData?: {
		...
		size?: number
		uploadData?: any
		refreshEvent?: (uploadFilesList: any, popupClose?: any) => void
	}
}
...
const accept = computed(() => {
	return props.compData.uploadData.accept || shallowRef<string>(['.json'].join(','))
}) // 文件格式
const multiple = computed(() => {
	return props.compData.uploadData.multiple
})
const limit = computed(() => {
	return props.compData.uploadData.limit
})
const header = { 'x-http-token': window.vite_public_request_token } // 请求头
const token = getToken()
const uploadUrl: string = window.location.protocol + '//' + window.location.host + (import.meta.env.DEV ? '/api/files?action=upload' : '/files?action=upload')
// const uploadUrl: string = 'https://124.71.211.126:14327/files?action=upload'

// '/www/backup/database/mysql/all_backup' 数据库备份上传
const fileData = reactive({
	f_size: 0,
	f_path: props.compData.path,
	f_name: '',
	f_start: 0,
...
const uploadSuccess = () => {
	...
	props.compData.refreshEvent && props.compData.refreshEvent(uploadFilesList.value, popupClose)
}
