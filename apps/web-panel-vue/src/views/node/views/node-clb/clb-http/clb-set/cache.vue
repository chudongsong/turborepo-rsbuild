...
import { cacheFormRules, formLoading, formDisabled, cacheFormRef, cacheData, timeUnit, cacheHelpList, setCacheEvent, onConfirmCache as onConfirm } from './useController'
import { useClbHttpStore } from '../useStore'

const { httpRowData } = useClbHttpStore()

watch(
	() => httpRowData.value.http_config,
	val => {
		if (val) {
			cacheData.status = val.proxy_cache_status
			let time = '1',
				unit = 'd'
			if (val.cache_time?.length > 1) {
				time = val.cache_time.slice(0, -1)
				unit = val.cache_time.slice(-1)
			}
			cacheData.time = time
			cacheData.unit = unit
