...
import { checkNodeFile } from '@api/node'
import { UploadFileItem, UploadStatus, FileUploadEventProps } from './type.d'
import { getByteUnit, getRandomChart } from '@utils/index'
...
export class UploadFile {
	...
	uploadFileItem = async (fileStart: number, index: number) => {
		...
		// 设置切割文件时间段
		const uploadedSize = this.uploadInfo.uploadedSize
		...
		form.append('node_id', this.nodeId.toString())
	...
	useFileUploadExistsConfirm = async () => {
		...
		const { data }: any = await checkNodeFile({
			files: list.join('\n'),
	 ...
	 */
	time = (date: any) => {
		let hours = Math.floor(date / (60 * 60 * 1000))
		let minutes = Math.floor(date / (60 * 1000))
		let seconds = (date % (60 * 1000)) / 1000
