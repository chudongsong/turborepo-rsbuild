<template>
	<div class="p-2rem">
		...
		<div v-if="form.edate" class="text-[#888] mt-[8px] ml-10rem">* 到期当天24点后过期</div>
	</div>
</template>
...
import { setEdate, setModulesEtimeMultiple, batchPhpEdate } from '@api/site'
...
const onConfirm = async (close?: any) => {
	try {
		if (isMult) {
			...
			// 组装批量设置到期时间参数
			const params = assembBatchParams(rowData, exclude, enable, { params: { edate: !form.edate ? '0000-00-00' : form.edate, project_type: 'PHP' } })
			// 批量设置到期时间
			const res: any = await batchPhpEdate(params)
			// 组装批量设置到期时间结果
			...
			if (siteType) {
				// 其他模块
				const res = await setModulesEtimeMultiple({
					sites_id: rowData.id,
