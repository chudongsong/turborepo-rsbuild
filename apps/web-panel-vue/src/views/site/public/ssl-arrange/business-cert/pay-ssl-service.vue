<template>
	<div class="px-[3rem] py-[2rem]">
		<div class="ssl-pay-type">
			<span v-for="item in tabsList" :key="item.label" :class="`ssl-pay-tab-item ${item.label === tabActive ? 'isActive' : ''}`" @click="cutPayType(item.label)">
				<span :class="`pay-icon ${item.label}`"></span>
				<span>{{ item.name }}</span>
			</span>
		</div>
		<div class="p-[32px] flex flex-col justify-center items-center mt-[16px]">
			<bt-qrcode :value="qrcodeUrl" :size="160" />
			<span class="mt-[20px] text-base font-bold flex items-center">
				<span>总计</span>
				<span class="text-[2.5rem] ml-[8px] text-orange">{{ orderInfo.price.toFixed(2) }} 元</span>
			</span>
		</div>
		<div class="ssl-pay-info">
			<div class="ssl-pay-info-line">
				<span class="w-[5rem]">商品名称</span>
				<span>证书安装服务</span>
			</div>
			<div class="ssl-pay-info-line">
				<span class="w-[5rem]">下单时间</span>
				<span>{{ orderTime }}</span>
			</div>
		</div>
		<div class="mt-[16px] flex items-center justify-center">
			<span class="scan-icon"></span>
			<span>
				{{ tabActive === 'wxcode' ? '微信扫一扫支付' : '支付宝扫一扫支付' }}
			</span>
		</div>
		<!-- <bt-dialog title="支付成功" v-model="paySuccessDialog" :area="60" :show-footer="false"> -->
		<!-- </bt-dialog> -->
	</div>
</template>

<script setup lang="ts">
import { formatTime } from '@/utils'
import { useSiteSSLStore } from '../useStore'

import { closeMonitorPayStatus, monitorPayServiceStatus, pollingMonitorPayServiceStatus } from './usePayController'

const { orderInfo } = useSiteSSLStore()

const props = withDefaults(
	defineProps<{
		compData: any
	}>(),
	{
		compData: {
			alicode: '',
			oid: 0,
			price: 0,
			qq: '',
			wxcode: '',
			wxoid: 0,
		},
	}
)
const emits = defineEmits(['close'])
const tabActive = ref('wxcode')

const orderTime = ref<any>(formatTime(new Date().getTime(), 'yyyy-MM-dd hh:mm:ss'))

const tabsList = [
	{ name: '微信支付', label: 'wxcode' },
	{ name: '支付宝支付', label: 'alicode' },
]

const qrcodeUrl = ref(props.compData[tabActive.value])

/**
 * @description 监听tab切换 props?????
 */
const cutPayType = (type: string) => {
	tabActive.value = type
	qrcodeUrl.value = props.compData[type]
}

onMounted(() => {
	pollingMonitorPayServiceStatus()
})

onUnmounted(() => {
	closeMonitorPayStatus()
})
</script>

<style lang="css" scoped>
.ssl-pay-type {
	@apply flex justify-center flex-row bg-base w-[40rem] mx-[auto] mt-[16px];
}
.ssl-pay-tab-item {
	@apply w-[20rem] h-[4rem] flex items-center justify-center cursor-pointer rounded-default text-base;
}
.ssl-pay-tab-item span {
	@apply flex h-[2rem] items-center;
}
.ssl-pay-tab-item .wxcode {
	background-image: url(data:image/png;base64,);
}
.ssl-pay-tab-item .alicode {
	background-image: url(data:image/png;base64,);
}
.ssl-pay-tab-item.isActive {
	@apply bg-primary text-white;
}
.ssl-pay-tab-item.isActive .wxcode {
	background-image: url(data:image/png;base64,);
}
.ssl-pay-tab-item.isActive .alicode {
	background-image: url(data:image/png;base64,);
}
.ssl-pay-tab-item .pay-icon {
	@apply flex bg-no-repeat w-[2rem] h-[2rem] bg-contain bg-center mr-[8px];
}
.ssl-pay-info {
	@apply w-[40rem] mx-[auto] py-[20px] px-[8px];
	border-top: 1px solid #ebeef5;
	border-bottom: 1px solid #ebeef5;
}
.ssl-pay-info .ssl-pay-info-line {
	@apply flex justify-between;
}
.ssl-pay-info .ssl-pay-info-line:first-child {
	@apply mb-[16px];
}
.scan-icon {
	@apply flex w-[2rem] h-[2rem] bg-no-repeat mr-[8px];
	background-size: 2rem;
	background-position: 0px 2px;
	background-image: url(data:image/png;base64,);
}
</style>
