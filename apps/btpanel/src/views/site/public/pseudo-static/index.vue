<template>
	<div class="h-full">
		<div v-if="isBindExtranet" v-bt-loading="textLoading" v-bt-loading:title="'正在加载内容，请稍候...'">
			<div v-if="['php', 'proxy', 'phpasync'].includes(siteType)" class="flex items-center justify-between">
				...
				<span class="flex items-center"
					>规则转换工具：
					<a class="bt-link" href="https://www.bt.cn/Tools" target="_blank">Apache转Nginx <i class="svgtofont-el-link"></i></a>
				</span>
			</div>
			<div class="my-[12px]">
				<bt-editor id="staticContentRef" :request="false" class="!h-[40rem] !w-full rounded-base" v-model="staticContent" @save="setReWriteBody()" />
			</div>
			<div>
		...
		<div class="bg-tertiary flex items-center justify-center h-full min-h-[60rem]" v-else>
...
import { textLoading, textValue, helpList, sitePath, optionData, staticContent, isClose, handleChange, initPseudoStatic, setReWriteBody, delPseudoStatic, setOtherTemplate } from './useController'
