<template>
	<div>
		<el-form ref="addThinkphpSiteFormRef" :disabled="formDisabled" :model="addSiteForm" :rules="rules">
			...
			<el-form-item label="PHP版本">
				<div class="flex">
					<div class="mr-2rem">
						<el-select class="!w-[16rem]" v-model="addSiteForm.php_version" @change="changeVersion">
							<el-option v-for="item in versionOptions" :key="item.value" :label="item.name" :value="item.version"></el-option>
						</el-select>
					</div>
					<el-form-item label="fileinfo版本">
						<bt-link v-if="!checkInfo.status" @click="openPlugin">当前php未安装fileinfo扩展 >>立即安装</bt-link>
						<span v-else class="!text-small">当前php已安装fileinfo扩展</span>
					</el-form-item>
				</div>
			</el-form-item>
...
import { GetPHPVersion, checkAutoInstall, checkInstallAsync, createAsync, getSystemUserListAsync } from '@api/site'
...
const addThinkphpSiteFormRef = ref<any>()
const formDisabled = ref(false)
const addSiteForm = ref({
	site_path: '/www/wwwroot',
	project_cmd: 'php think run',
	install_dependence: 0,
	php_version: '',
	thinkphp_version: true,
	domains: '',
	sql: false,
	project_ps: '',
	project_port: '',
...
const helpList = [
	...
	{ content: '【fileinfo版本】若未安装，项目会启动失败' },
	{
		content: (
			<>
				【自动安装项目依赖】只尝试
...
const onConfirm = async (close: any) => {
	await addThinkphpSiteFormRef.value.validate()
	if (!checkInfo.value.status) {
		await useConfirm({
			width: 50,
			title: `提示`,
			isHtml: true,
			content: '当前版本PHP-安装扩展未安装fileinfo，可能导致项目无法启动，是否继续操作？',
			icon: 'warning-filled',
		})
	}
