<template>
	<div class="mb-16px font-bold text-medium text-default">点击率%</div>
...
onResult(data => {
	const { click_rate_chart: chart } = data
	if (chart) {
		const rateData = chart.data.map(item => {
			return [getChartTime(chart.column_type, item.x), item.click_rate] as [string, number]
		})
