import {
	...
	delProxyUrlData,
	delProxyUrlReplace,
	editProxyUrlDataRemark,
	editProxyUrlInfo,
	getProxyUrlData,
	proxyUrlAddLimit,
	proxyUrlBatchDelLimit,
	proxyUrlCache,
	proxyUrlCompress,
	proxyUrlDelLimit,
	setProxyUrlCustom,
...
export const getUrlList = async () => {
	try {
		const { data: res }: ResponseResult = await getProxyUrlData({
...
export const addUrlEvent = () => {
	useDialog({
		...
		area: 60,
		btn: '确认',
		component: () => import('@docker/views/docker-site/setting/url-proxy/add-url-proxy.vue'),
	})
}
...
export const openSetting = (row: any) => {
	...
	useDialog({
		isAsync: true,
		title: `反向代理设置【${row.proxy_path}】`,
		area: [76, 60],
		component: () => import('@docker/views/docker-site/setting/url-proxy/setting.vue'),
		compData: {
			row,
		},
	})
...
export const onPathChange = () => {
	fileSelectionDialog({
		type: 'file',
		path: urlData.proxysite || '/www/dk_project/wwwroot',
		change: (path: string) => {
...
/**
 * @description 初始化urlData
 */
export const initUrlData = () => {
	urlData.targetType = 'http'
	urlData.sitename = ''
	urlData.cachetime = 1
	urlData.proxysite = 'http://'
	urlData.todomain = '$http_host'
	urlData.path = ''
	urlData.ps = ''
	urlData.websocket = false
}
 ...
 * @param close
 */
export const onConfirmAdd = async (close?: any) => {
...
export const addReplaceView = () => {
	...
	useDialog({
		...
		area: 60,
		btn: '确认',
		component: () => import('@docker/views/docker-site/setting/url-proxy/setting-replace-add.vue'),
		compData: {
			proxy_path: proxyData.value.proxy_path,
 ...
 * @description: 提交表单
 */
export const onConfirmReplace = async (close?: Function) => {
...
export const bufferData = reactive({
	status: false, // 缓存状态
})
// /**
//  * @description 获取状态
...
// 	await useDataHandle({
// 		loading: formDisabled,
// 		request: getDockerSiteBuffer({
// 			site_name: siteInfo.value.name,
// 		}),
// 		success: (res: any) => {
// 			if(res.status){
// 				bufferData.status = !!res.data.status
// 			}else{
// 				Message.error(res.msg)
// 			}
// 		}
 ...
 * @param close
 */
export const onConfirmBuffer = async (val: boolean) => {
	const params = {
		site_name: siteInfo.value.name,
		proxy_path: proxyData.value.proxy_path,
		status: (val ? 1 : 0) as 0 | 1,
	}
	await useDataHandle({
		loading: '正在保存，请稍后...',
		request: setDockerSiteBuffer(params),
		message: true,
	})
}
export const bufferHelpList = [
	{
		content: '启用缓冲（默认）：Nginx 会将后端服务器（被代理的服务器）的响应数据先存储在自己的缓冲区中',
	},
	{
		content: '启用缓冲（默认）：提高性能和稳定性，优化资源利用，但也会造成延迟问题，内存占用高',
	},
	{
		content: '关闭缓冲：数据访问实时性增强，降低内存占用风险，但也会对后端服务器（被代理的服务器）压力增大，性能也有一定影响',
	},
	{
		content: '什么业务可以关闭缓冲：例如AI类型的，比如ThinkChat等',
	},
]
/*********  设置buffer end  ************/
	...
	return [
		{
			...
			event: async (batchConfirm, nextAll, selectedList, options) => {
				deleteIpEvent(selectedList.value)
			},
		},
	]
}
...
export const initIpData = () => {
	...
	ipData.value.types = listData.type
...
export const addIpView = () => {
	useDialog({
		...
		area: 40,
		btn: '确认',
		component: () => import('@docker/views/docker-site/setting/url-proxy/setting-ip-add.vue'),
		compData: {
 ...
 * @description: 提交表单
 */
export const onConfrimIp = async (close: Function) => {
	const { types: ip_type, address: ips } = ipData.value
	const params = {
		site_name: siteInfo.value.name,
		proxy_path: proxyData.value.proxy_path,
		ip_type,
