...
const { siteInfo: dockerSiteInfo } = useDockerSiteStore() // 站点信息

const { BtTabs } = useTabs({
	type: 'card',
	options: tabComponent.filter(option => {
		// docker项目隐藏测试证书
		return option.name !== 'trustAsiaList'
	}),
	value: sslTabActive,
})

onMounted(() => {
	siteInfo.value = { ...dockerSiteInfo.value, project_type: 'docker' }
	siteType.value = 'docker'
	getSslInfoConfig()
})
</script>
