...
const { versionOptions, extendOptions, isEdit, rowData } = storeToRefs(store)
...
export const getFormData = async (dataRef: Ref<{ version: string; repo_url: string }>): Promise<any> => {
	try {
		const res: any = await getVersion('python')
		if (res.status) {
			versionOptions.value = res.data['python'].map((item: any) => ({ label: item, value: item }))
 ...
 * @returns {App}
 */
export const openPythonForm = (row?: any): Promise<boolean> => {
	...
	return new Promise((resolve, error) => {
		useDialog({
			...
			area: 80,
			btn: ['确认', '取消'],
			component: () => import('@docker/views/docker-site/site-env/python-env/python-form.vue'),
			onCancel: () => {
				resetData()
				resolve(false)
			},
 ...
 * @description 创建环境
 */
export const createPythonEnv = async (formData: any, close: AnyFunction): Promise<any> => {
	try {
		...
		await useDataHandle({
			...
			success: (res: any) => {
				if (res.status) {
					openBuildLog(`tail -f /www/dk_project/runtime/run/python/${formData.name}/create.log`, () => {
						setTypeRefreshTable('python')
					})
					close()
				} else {
