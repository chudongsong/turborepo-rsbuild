<template>
	<div id="safeDetectList" v-bt-loading="loading">
		<div class="text-large mb-6">服务器安全检测</div>
		<div class="progress-header">
			<div class="progress-header-cot">
				<div :class="'progresscirclebar ' + (!scanDetect.status ? '' : 'security ') + 'text-[' + (scanDetect.msg?.security_count === 100 ? ' var(--el-color-primary) ' : ' #fc6d26 ') + ']'">
					...
					<template v-else>
						<div v-if="scanParam.scanStatus && !scanParam.isEnd" class="relative">
							<el-progress class="animate-spin" type="circle" :percentage="70" :show-text="false" :stroke-width="4" :define-back-color="'#fff'" :color="scanParam.warn || scanParam.error ? '#fc6d26' : 'var(--el-color-primary)'" :width="100"></el-progress>
						...
						<el-progress v-else type="circle" :percentage="scanDetect.msg?.security_count" :format="format" :color="scanDetect.msg?.security_count === 100 ? 'var(--el-color-primary)' : '#fc6d26'" :width="100"></el-progress>
					</template>
				</div>
			</div>
			<div class="progress-header-cot">
				...
				<el-progress v-if="scanParam.scanStatus" :percentage="scanParam.progress" :color="scanParam.warn || scanParam.error ? '#fc6d26' : 'var(--el-color-primary)'" :stroke-width="10"></el-progress>
		...
		<div class="progress-cont-list">
			<div v-for="(value, key) in progressType" :key="key" :class="key + '_item ' + (scanParam.active[key] ? 'active' : '')" class="progress_item">
				<div class="progress_item_header" @click="titleClick(key)">
					...
					<div class="progress_fold" v-if="scanParam.progressConfig[key] && scanParam.progressConfig[key].length">
						<bt-image :src="'/firewall/arrow-down.svg'" class="mt-[.6rem]" />
					</div>
				</div>
...
const onWSReceive = (ws: WebSocket, e: MessageEvent) => {
	if (e.data) {
		...
		if (msg.hasOwnProperty('callback')) {
			...
			// 按status的结果3优先，1最后
			scanParam.progressConfig[msg.topic].sort((a: any, b: any) => {
			...
			if (msg.status !== 1) {
				scanParam.progressColor = '#fc6d26'
			}
		}
	}
}
...
.progress_item.active :deep(.progress_fold .el-image) {
