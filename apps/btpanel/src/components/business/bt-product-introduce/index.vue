		...
		<div class="flex items-start">
			...
			<div class="flex items-start flex-col">
				...
				<div class="product-introduce-groupBtn">
					...
					<el-button v-if="(!data.pluginInfo?.setup || !data.isInstall) && (data.pluginInfo?.endtime >= 0 || data.isSkipPay)" type="primary" @click="installSoftModel"> 立即安装 </el-button>
					<el-button v-else-if="!data.isSkipPay" type="primary" @click="payClick"> 立即购买 </el-button>
				</div>
			</div>
		</div>
...
const installSoftModel = async (item: AnyObject) => {
	if (!props.data.pluginInfo && !props.data.name) return
	let name = props.data?.name || ''
	if (hasOwnProperty(props.data.pluginInfo, 'name')) name = props.data.pluginInfo?.name
	pluginInstallDialog({
		name,
		type: 'i',
		softData: {
